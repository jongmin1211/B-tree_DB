...
class Node:
    def __init__(
        self, m: int = 1, p: list = None, r: int = -1, isLeaf=True, offset: int = None
    ):
        self.m = m  # # of keys
...
def readFreeNode(offset) -> FreeNode:
    ...
    return FreeNode(
        offset, nextOffset
    )  # offset : the one we seeked to, not read from file
...
def readNode(offset) -> Node:
    ...
    p = [list(pair) for pair in zip(p[::2], p[1::2])][
        :m
    ]  # make pair, drop unused slots
...
def releaseOffset(
    f, offset
):  # release offset and make link to linked list of FreeNode.
...
def split(
    n: Node, path: list
):  # when node overflowed, split node and make parent node, keep left node and create right node.
...
def insert(inputFile):
    ...
    INPUT_CSV = os.path.join(HERE, inputFile)
...
def distribute(parent, left, right, n, i):
    # minimum number of KEYS. a non-leaf keeps ceil(N/2) children, so one key less.
    ...
    if left and left.m - 1 >= minKeys:
        ...
        raise RuntimeWarning(
            f"unexpected node in distribute left : {left}, right : {right}, n : {n}"
        )
...
def rebalance(n, trace):
    ...
    if n.m < minKeys:
        ...
        if left and left.m + n.m + extra < N:
            ...
            distribute(parent, left, right, n, i)

        if parent.m + 1 < ceil(N / 2):
...
def delete(deleteFile):
    # TODO
    DELETE_CSV = os.path.join(HERE, deleteFile)
...
with open(ifile, "r+b") as f:
    ...
    modified: dict[int, list] = dict()
      # dictionary of modified node {offset : [node, 'stat']}
